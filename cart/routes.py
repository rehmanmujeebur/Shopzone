import os
import json
# import stripe  # 🚫 Stripe temporarily disabled
...
def checkout():
    ...
    return render_template('checkout.html',
                           cart_items=cart_items,
                           total=total)
                           # , stripe_public_key=stripe_public_key)  # 🚫 Disabled Stripe
...
def remove_from_cart(product_id):
    cart_items = session.get('cart_items', [])
    new_cart = [item for item in cart_items if item['product']['id'] != product_id]
